...
int Leaf::getPrecedence() const
{
    return static_cast<int>(Precedence::VALUE);
}
...
BinaryOperator::BinaryOperator(Op op, const Node::Ptr& left, const Node::Ptr& right) :
    op(op),
    left(left->clone()),
    right(right->clone())
{}
...
Data::Data(const std::string& table, const std::string& field) :
    root(Node::make<Leaf>(table, field)),
    tables({table})
{}
...
Data::Data(BinaryOperator::Op op, const Data& left, const Data& right) :
    root(Node::make<BinaryOperator>(op, left.root->clone(), right.root->clone())),
    tables(left.tables),
    binds(left.binds)
{
...
Data::Data(BinaryOperator::Op op, Data&& left, const Data& right) :
    root(Node::make<BinaryOperator>(op, move(left.root), right.root->clone())),
...
Data::Data(BinaryOperator::Op op, const Data& left, Data&& right) :
    root(Node::make<BinaryOperator>(op, left.root->clone(), move(right.root))),
