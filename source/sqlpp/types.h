...
struct TypeName<Text> {
  static std::string get() { return "TEXT"; }
};

template <>
struct TypeName<Blob> {
  static std::string get() { return "BLOB"; }
};
...
template <typename V>
struct Converter<
    V, std::enable_if_t<std::is_same_v<std::remove_cvref_t<V>, Integer> ||
  ...
  static DbType toDb(const DbType& value) { return value; }
  static DbType fromDb(const DbType& value) { return value; }
};

template <typename V>
struct Converter<
    V, std::enable_if_t<std::is_integral_v<std::remove_cvref_t<V>> &&
                        !std::is_same_v<std::remove_cvref_t<V>, Integer>>> {
  using DbType = Integer;
  static Integer toDb(const V& value) { return static_cast<Integer>(value); }
  static V fromDb(const Integer& value) { return static_cast<V>(value); }
};

template <typename V>
struct Converter<
    V, std::enable_if_t<std::is_floating_point_v<std::remove_cvref_t<V>> &&
                        !std::is_same_v<std::remove_cvref_t<V>, Real>>> {
  using DbType = Real;
  static Real toDb(const V& value) { return static_cast<Real>(value); }
  static V fromDb(const Real& value) { return static_cast<V>(value); }
};
...
auto createBind(V&& value) {
  return [v = toDb(std::forward<V>(value))](sqlite3_stmt* stmt, int idx) {
    bind(stmt, idx, v);
  };
}
...
struct ContainsS<U, List<T...>> {
  static constexpr bool value =
      (std::is_same_v<Head<U>, Head<List<T...>>> ||
       Contains<Head<U>, Tail<List<T...>>>)&&Contains<Tail<U>, List<T...>>;
};
...
struct AddS<U, List<T...>> {
  using Type =
      std::conditional_t<Contains<U, List<T...>>, List<T...>, List<U, T...>>;
  ;
};
...
template <>
struct IntList<> {
  static constexpr bool contains(size_t) { return false; }
};
