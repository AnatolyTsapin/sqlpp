...
int main(int argc, char* argv[]) {
  ...
  auto stmt =
      sqlpp::insertValues(test.id <<= 0, test.comment <<= "0"s, test.id <<= 1);
#endif
  ...
  auto stmt = sqlpp::select(test.id).where(
      test.comment = another.value || test.value < 0 && another.id > 0);
#endif

#ifdef CHECK_SELECT_CONDITION_FAIL
  auto stmt =
      sqlpp::select(test).where(test.id == 0L && test.comment > test.value);
#endif
